...
1. Look up the compound in `TH_DEMO_DB.HOL_2_PHARMA.COMPOUNDS` by name
...
WITH compound_info AS (
    SELECT * FROM TH_DEMO_DB.HOL_2_PHARMA.COMPOUNDS
    WHERE LOWER(compound_name) = LOWER('<COMPOUND_NAME>')
),
trial_info AS (
    SELECT t.*
    FROM TH_DEMO_DB.HOL_2_PHARMA.CLINICAL_TRIALS t
    JOIN compound_info c ON t.compound_id = c.compound_id
),
ae_info AS (
    SELECT a.*
    FROM TH_DEMO_DB.HOL_2_PHARMA.ADVERSE_EVENTS a
